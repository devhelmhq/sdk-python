...
def _monitor(**kw: object) -> dict:
    base: dict = {
        "id": UID, "organizationId": 1, "name": "M", "type": "HTTP",
        "config": {"url": "https://a.com", "method": "GET"},
        "frequencySeconds": 60, "enabled": True, "regions": ["us-east"],
        "managedBy": "DASHBOARD", "createdAt": NOW, "updatedAt": NOW,
    }
    base.update(kw)
    return base
...
def _incident(**kw: object) -> dict:
    base: dict = {
        "id": UID, "organizationId": 1, "source": "MANUAL",
        "status": "TRIGGERED", "severity": "DOWN", "affectedRegions": [],
        "reopenCount": 0, "statusPageVisible": False,
...
def _alert_channel(**kw: object) -> dict:
    base: dict = {
        "id": UID, "name": "ch", "channelType": "slack",
...
def _environment(**kw: object) -> dict:
    base: dict = {
        "id": UID, "orgId": 1, "name": "prod", "slug": "prod",
        "variables": {}, "createdAt": NOW, "updatedAt": NOW,
        "monitorCount": 0, "isDefault": False,
    }
    base.update(kw)
    return base
...
def _secret(**kw: object) -> dict:
    base: dict = {
        "id": UID, "key": "MY_KEY", "dekVersion": 1,
        "valueHash": "abc123", "createdAt": NOW, "updatedAt": NOW,
    }
    base.update(kw)
    return base
...
def _tag(**kw: object) -> dict:
    base: dict = {
        "id": UID, "organizationId": 1, "name": "prod", "color": "#000000",
        "createdAt": NOW, "updatedAt": NOW,
    }
    base.update(kw)
    return base
...
def _webhook(**kw: object) -> dict:
    base: dict = {
        "id": UID, "url": "https://hook.example.com",
...
def _deploy_lock(**kw: object) -> dict:
    base: dict = {
        "id": UID, "lockedBy": "ci-job-42",
        "lockedAt": NOW, "expiresAt": NOW,
    }
    base.update(kw)
    return base
...
def _resource_group(**kw: object) -> dict:
    base: dict = {
        "id": UID, "organizationId": 1, "name": "rg", "slug": "rg",
        "suppressMemberAlerts": False,
        "health": {
            "status": "operational", "totalMembers": 0,
            "operationalCount": 0, "activeIncidents": 0,
        },
        "createdAt": NOW, "updatedAt": NOW,
    }
    base.update(kw)
...
def _notification_policy(**kw: object) -> dict:
    base: dict = {
        "id": UID, "organizationId": 1, "name": "np",
        "matchRules": [], "escalation": {
            "steps": [{"delayMinutes": 0, "channelIds": [UID]}],
        },
...
def _status_page(**kw: object) -> dict:
    base: dict = {
        "id": UID, "organizationId": 1, "workspaceId": 1,
        "name": "SP", "slug": "sp",
        "branding": {}, "visibility": "PUBLIC", "enabled": True,
        "incidentMode": "MANUAL",
        "createdAt": NOW, "updatedAt": NOW,
    }
    base.update(kw)
...
def _sp_component(**kw: object) -> dict:
    base: dict = {
        "id": UID, "statusPageId": UID, "name": "API", "type": "STATIC",
        "currentStatus": "OPERATIONAL", "showUptime": True,
        "displayOrder": 0, "pageOrder": 0, "excludeFromOverall": False,
        "createdAt": NOW, "updatedAt": NOW,
    }
    base.update(kw)
    return base
...
def _sp_group(**kw: object) -> dict:
    base: dict = {
        "id": UID, "statusPageId": UID, "name": "Infra",
        "displayOrder": 0, "pageOrder": 0, "collapsed": True,
        "createdAt": NOW, "updatedAt": NOW,
    }
    base.update(kw)
    return base
...
def _sp_incident(**kw: object) -> dict:
    base: dict = {
        "id": UID, "statusPageId": UID, "title": "Down",
...
def _sp_incident_update(**kw: object) -> dict:
    base: dict = {
        "id": UID, "status": "INVESTIGATING", "body": "Looking into it",
        "notifySubscribers": True, "createdAt": NOW,
    }
    base.update(kw)
    return base
...
def _sp_subscriber(**kw: object) -> dict:
    base: dict = {
        "id": UID, "email": "a@b.com", "confirmed": True, "createdAt": NOW,
    }
    base.update(kw)
    return base
...
def _sp_custom_domain(**kw: object) -> dict:
    base: dict = {
        "id": UID, "hostname": "status.example.com",
        "status": "ACTIVE", "verificationMethod": "CNAME",
        "verificationToken": "tok123", "verificationCnameTarget": "verify.example.com",
        "createdAt": NOW, "updatedAt": NOW, "primary": True,
    }
    base.update(kw)
    return base
...
def _sp_incident_component(**kw: object) -> dict:
    base: dict = {
        "statusPageComponentId": UID, "componentStatus": "OPERATIONAL",
        "componentName": "API",
    }
    base.update(kw)
    return base
...
def _check_result(**kw: object) -> dict:
    base: dict = {
        "id": UID, "timestamp": NOW, "region": "us-east",
        "passed": True,
    }
    base.update(kw)
    return base
...
def _incident_policy(**kw: object) -> dict:
    base: dict = {
        "id": UID, "monitorId": UID,
        "triggerRules": [{
            "type": "consecutive_failures", "scope": "per_region",
            "severity": "down",
        }],
        "confirmation": {
            "type": "ALL_REGIONS", "minRegionsFailing": 1, "maxWaitSeconds": 30,
        },
        "recovery": {
            "consecutiveSuccesses": 1, "minRegionsPassing": 1, "cooldownMinutes": 5,
        },
        "createdAt": NOW, "updatedAt": NOW,
    }
    base.update(kw)
...
def _monitor_version(**kw: object) -> dict:
    base: dict = {
        "id": UID, "monitorId": UID, "version": 1,
        "snapshot": _monitor(), "changedVia": "DASHBOARD",
        "createdAt": NOW,
    }
    base.update(kw)
    return base
...
class TestCreateMonitorRequestNegative:
    def test_missing_name(self) -> None:
        with pytest.raises(ValidationError, match="name"):
            CreateMonitorRequest.model_validate({
                "type": "HTTP", "config": {"url": "https://x.com", "method": "GET"},
                "managedBy": "DASHBOARD",
            })
    ...
    def test_missing_type(self) -> None:
        with pytest.raises(ValidationError, match="type"):
            CreateMonitorRequest.model_validate({
                "name": "X", "config": {"url": "https://x.com", "method": "GET"},
                "managedBy": "DASHBOARD",
            })
...
class TestCreateManualIncidentRequestNegative:
    ...
    def test_empty_title(self) -> None:
        with pytest.raises(ValidationError):
            CreateManualIncidentRequest.model_validate({"title": "", "severity": "DOWN"})
    ...
    def test_invalid_severity(self) -> None:
        with pytest.raises(ValidationError):
            CreateManualIncidentRequest.model_validate({"title": "X", "severity": "SEV1"})

    def test_wrong_title_type(self) -> None:
        with pytest.raises(ValidationError):
            CreateManualIncidentRequest.model_validate({"title": [1], "severity": "DOWN"})
    ...
    def test_null_title(self) -> None:
        with pytest.raises(ValidationError):
            CreateManualIncidentRequest.model_validate({"title": None, "severity": "DOWN"})
...
class TestCreateNotificationPolicyRequestNegative:
    ...
    def test_null_name(self) -> None:
        with pytest.raises(ValidationError):
            CreateNotificationPolicyRequest.model_validate({
                "name": None, "matchRules": [], "escalation": {
                    "steps": [{"delayMinutes": 0, "channelIds": [UID]}],
                }, "enabled": True, "priority": 0,
            })
    ...
    def test_wrong_match_rules_element(self) -> None:
        with pytest.raises(ValidationError):
            CreateNotificationPolicyRequest.model_validate({
                "name": "np", "matchRules": ["bad"], "escalation": {
                    "steps": [{"delayMinutes": 0, "channelIds": [UID]}],
                }, "enabled": True, "priority": 0,
            })
    ...
    def test_missing_consecutive_failures(self) -> None:
        with pytest.raises(ValidationError, match="consecutiveFailures"):
            WebhookEndpointDto.model_validate(
                _del(_webhook(), "consecutiveFailures")
            )

    def test_wrong_consecutive_failures_type(self) -> None:
        with pytest.raises(ValidationError):
            WebhookEndpointDto.model_validate(
                _webhook(consecutiveFailures="many")
            )
...
class TestStatusPageComponentDtoNegative:
    ...
    def test_invalid_current_status(self) -> None:
        with pytest.raises(ValidationError):
            StatusPageComponentDto.model_validate(
                _sp_component(currentStatus="BROKEN")
            )
    ...
    def test_missing_display_order(self) -> None:
        with pytest.raises(ValidationError, match="displayOrder"):
            StatusPageComponentDto.model_validate(
                _del(_sp_component(), "displayOrder")
            )
    ...
    def test_missing_confirmed(self) -> None:
        with pytest.raises(ValidationError, match="confirmed"):
            StatusPageSubscriberDto.model_validate(
                _del(_sp_subscriber(), "confirmed")
            )

    def test_missing_created_at(self) -> None:
        with pytest.raises(ValidationError, match="createdAt"):
            StatusPageSubscriberDto.model_validate(
                _del(_sp_subscriber(), "createdAt")
            )
...
class TestStatusPageCustomDomainDtoNegative:
    ...
    def test_invalid_status(self) -> None:
        with pytest.raises(ValidationError):
            StatusPageCustomDomainDto.model_validate(
                _sp_custom_domain(status="MAGIC")
            )
...
class TestConfirmationPolicyNegative:
    ...
    def test_wrong_min_regions_type(self) -> None:
        with pytest.raises(ValidationError):
            ConfirmationPolicy.model_validate({
                "type": "ALL_REGIONS", "minRegionsFailing": "all",
                "maxWaitSeconds": 30,
            })
...
class TestRecoveryPolicyNegative:
    ...
    def test_wrong_consecutive_type(self) -> None:
        with pytest.raises(ValidationError):
            RecoveryPolicy.model_validate({
                "consecutiveSuccesses": "two", "minRegionsPassing": 1,
                "cooldownMinutes": 5,
            })
    ...
    def test_missing_incident(self) -> None:
        with pytest.raises(ValidationError, match="incident"):
            IncidentDetailDto.model_validate({
                "updates": [], "statusPageIncidents": [],
            })

    def test_wrong_incident_type(self) -> None:
        with pytest.raises(ValidationError):
            IncidentDetailDto.model_validate({
                "incident": "not-a-dict", "updates": [],
                "statusPageIncidents": [],
            })
    ...
    def test_missing_monitor_id(self) -> None:
        with pytest.raises(ValidationError, match="monitorId"):
            MonitorVersionDto.model_validate(
                _del(_monitor_version(), "monitorId")
            )

    def test_missing_version(self) -> None:
        with pytest.raises(ValidationError, match="version"):
            MonitorVersionDto.model_validate(
                _del(_monitor_version(), "version")
            )
...
class TestDashboardOverviewDtoNegative:
    ...
    def test_missing_incidents(self) -> None:
        with pytest.raises(ValidationError, match="incidents"):
            DashboardOverviewDto.model_validate({
                "monitors": {"total": 0, "healthy": 0, "degraded": 0, "down": 0, "disabled": 0, "paused": 0},
            })
